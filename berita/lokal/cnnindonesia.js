...
function parse(xml, limit = 10) {
  ...
  const items = [...channel.matchAll(/<item\b[^>]*>([\s\S]*?)<\/item>/gi)].map(([, item]) => ({
    ...
    imageType: attr(item, 'enclosure', 'type'),
    imageLength: attr(item, 'enclosure', 'length'),
    summary: tag(item, 'content:encoded').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim(),
    descriptionHtml: tag(item, 'description')
  }));
  const latest = items.sort((a, b) => Date.parse(b.publishedAt) - Date.parse(a.publishedAt)).slice(0, limit);
  return latest;
}
...
(async () => {
  try {
    ...
    const data = parse(await get(RSS_URL), limit);
    console.log(JSON.stringify(data, null, 2));
  } catch (error) {
    console.error(`Gagal mengambil RSS CNN Indonesia: ${error.message}`);
    process.exitCode = 1;
  }
})();
