...
const RSS_URL = 'https://www.antaranews.com/rss/terkini.xml';
...
function parse(xml, limit = 10) {
  ...
  const items = [...channel.matchAll(/<item\b[^>]*>([\s\S]*?)<\/item>/gi)].map(([, item]) => ({
    ...
    summary: tag(item, 'content:encoded') || tag(item, 'description').replace(/<[^>]*>/g, ' ')
  }))
    .map(item => ({ ...item, summary: item.summary.replace(/\s+/g, ' ').trim() }));
...
(async () => {
  try {
    ...
    console.error(`Gagal mengambil ANTARA: ${error.message}`);
    process.exitCode = 1;
  }
})();
